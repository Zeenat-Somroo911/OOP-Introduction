...
const main = async () => {
  try {
    const myName = await askQuestion(
      chalk.bold.greenBright("What is your name? ")
    );
    ...
    const answer = await askQuestion(
      chalk.bold.greenBright(
        chalk.bold.yellowBright("Type 1: ") +
    ...
    if (isNaN(answerNum) || answerNum < 1 || answerNum > 3) {
      console.log(
        chalk.bold.redBright("Invalid input. Please enter 1, 2, or 3.")
      );
    } else {
